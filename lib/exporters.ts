...
export async function exportToGIF(
  ...
  try {
    ...
    // Get the model group first
    const modelGroup = modelGroupRef.current;
    if (!modelGroup) {
      console.error("Model group not found in ref");
      throw new Error("3D model not found. Please ensure the model is loaded.");
    }

    console.log("Model group found, current rotation:", modelGroup.rotation.y);
    ...
    const originalRotation = modelGroup.rotation.y;
    console.log("Original rotation stored:", originalRotation);

    // Generate frames by capturing canvas at different rotation angles
    for (let i = 0; i < totalFrames; i++) {
      // Calculate rotation for this frame
      const rotationProgress = i / totalFrames;
      const rotationAngle = rotationProgress * Math.PI * 2; // Full 360 degree rotation
      
      // Apply rotation to model
      modelGroup.rotation.y = originalRotation + rotationAngle;
      
      // Wait for the render loop to update
      // This gives React Three Fiber time to process the rotation change
      ...
      // Show progress (only at 25%, 50%, 75% and completion)
      const progress = Math.round((i / totalFrames) * 100);
      if (progress === 25 || progress === 50 || progress === 75 || i === totalFrames - 1) {
        console.log(`Capturing frame ${i + 1}/${totalFrames} (${progress}%)`);
        if (i === totalFrames - 1) {
          toast.info("Frames captured, encoding GIF...", { duration: 3000 });
        } else {
          toast.info(`Creating GIF: ${progress}% complete`, { duration: 1000 });
        }
      }
    }

    // Restore original rotation
    modelGroup.rotation.y = originalRotation;
    console.log("Rotation restored to:", originalRotation);
    ...
    const blob = await new Promise<Blob>((resolve, reject) => {
      ...
      gif.on('progress', (progress: number) => {
        ...
        // Only log encoding progress, no toast notifications
        if (percent % 25 === 0) {
    ...
    toast.error(`Failed to create animated GIF: ${(error as Error).message}`);
    return false;
  }
}
...
export async function handleGIFExport(
  ...
  if (!modelGroupRef.current || !fileName) {
    console.error("GIF Export failed: Model group or filename missing");
    toast.error("Error: Cannot export GIF - model not loaded");
    return;
  }

  try {
    const success = await exportToGIF(
      modelGroupRef, 
      baseName, 
      duration, 
      fps, 
    ...
    if (success) {
      toast.success(`${baseName}_animated.gif has been downloaded successfully`, {
        duration: 3000,
      });
    } else {
    ...
    toast.error(
      `GIF export failed: ${(error as Error).message || "Unknown error"}`,
    );
  }
}
